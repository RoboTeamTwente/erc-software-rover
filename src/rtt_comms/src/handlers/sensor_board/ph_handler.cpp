...
: node_(node)
{
  pub_ = node_->create_publisher<rtt_comms::msg::SensorBoardPHInfo>(topic, queue_size);
}
...
void PhHandler::handle(const PBEnvelope& envelope) {
  ...
  rtt_comms::msg::SensorBoardPHInfo ph_ros;
  ph_ros.ph_value       = ph_pb.ph_value();
  ph_ros.voltage        = ph_pb.voltage();
  ph_ros.temperature    = ph_pb.temperature();

  ph_ros.state.state    = rtt_comms::udp::clamp_u8(static_cast<int>(ph_pb.state()));
  ph_ros.error_code     = rtt_comms::udp::clamp_u8(static_cast<int>(ph_pb.error_code()));
