...
public:
  ...
  UdpForwarderNode()
    ...
    dst_ip_      = this->declare_parameter<std::string>(
        "dst_ip", "127.0.0.1");
    ...
    handlers_.emplace(
        ...
        std::make_unique<DiagnosticsHandler>(
            this,
            "sensor_board_diagnostics",
            10));
  ...
  void on_task_received(
      const std::string& task_name,
      const std::string& target_id) {

    if (!bt_client_->wait_for_action_server(
            std::chrono::seconds(1))) {
      RCLCPP_WARN(
          get_logger(),
          "BT Manager action server not available");
      return;
    }
    ...
    auto opts =
        rclcpp_action::Client<CollectTask>::SendGoalOptions{};
    ...
    opts.feedback_callback =
        [this](
            ...
            const std::shared_ptr<
                const CollectTask::Feedback> feedback) {
...
private:
  void handle_hw_accepted(
    ...
    std::thread([this, handle]() {
      ...
      RCLCPP_INFO(
          get_logger(),
          "HW command received: [%s] param=%.2f",
          goal->command_type.c_str(),
          goal->param_float);

      // TODO comms to hardware to comms communication

      // Stub hardware latency
      std::this_thread::sleep_for(
          std::chrono::milliseconds(300));

      auto result =
          std::make_shared<HardwareCommand::Result>();
  ...
  void rx_loop() {
    ...
    while (running_ && rclcpp::ok()) {
      ...
      // Forward raw datagram
      ::sendto(
          sock_,
          buffer.data(),
          n,
          0,
          reinterpret_cast<sockaddr*>(&dstA_),
          sizeof(dstA_));
      ...
      ::sendto(
          ...
          n,
          0,
          reinterpret_cast<sockaddr*>(&dstB_),
          sizeof(dstB_));

      // Parse datagram as PBEnvelope
      PBEnvelope envelope;
      ...
      if (!envelope.ParseFromArray(
        ...
        RCLCPP_WARN_THROTTLE(
            ...
            *get_clock(),
            2000,
            "Failed to parse PBEnvelope (%zd bytes)",
            n);

        continue;
      }

      const int payload_case =
      ...
      if (it == handlers_.end()) {
        ...
        RCLCPP_WARN_THROTTLE(
            ...
            *get_clock(),
            2000,
            "No handler for PBEnvelope payload_case=%d",
            payload_case);
...
private:
  ...
  int listen_port_{5000};
  int dst_a_port_{6000};
  int dst_b_port_{6001};
  ...
  std::unordered_map<
      int,
      std::unique_ptr<Handler>> handlers_;

  rclcpp_action::Client<CollectTask>::SharedPtr
      bt_client_;

  rclcpp_action::Server<HardwareCommand>::SharedPtr
      hw_server_;
};
...
int main(int argc, char** argv) {
  ...
  auto node =
      std::make_shared<UdpForwarderNode>();
